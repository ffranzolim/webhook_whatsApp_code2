...
class WhatsApp {
    ...
    */
    async _send(payload) {
        ...
        if (self._isTextOrLocationMessageWithoutActions(type, actions, globalActions)) {
          ...
          // Handle card message
          // verifica se são multiplos cards e se for faz o loop
          var qtdCards = messagePayload.cards.length; 
          for(var i=0;i<qtdCards;i++) {
            var card = messagePayload.cards[i];
            await self._handleCardMessage([card], globalActions, headerText, footerText, data);
            //await self.whatsAppSender._sendMessageToWhatsApp(data);
            await self._sendToWhatsApp(data);
          }
          return;
    ...
    */
    async _sendToWhatsApp(message) {
        let self = this;
       // self.whatsAppSender._queueMessage(message);
       await self.whatsAppSender._sendMessageToWhatsApp(message);
    }
};
